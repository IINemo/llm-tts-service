...
def get_model_architecture(model_name: str) -> Optional[ModelArchitecture]:
    ...
    try:
        ...
        hidden_size = getattr(config, "hidden_size", None) or getattr(config, "d_model", 4096)
        num_layers = getattr(config, "num_hidden_layers", None) or getattr(config, "n_layer", 36)
        num_heads = getattr(config, "num_attention_heads", None) or getattr(config, "n_head", 32)
        ...
        log.info(f"Loaded architecture for {model_name}: {num_layers} layers, h={hidden_size}, ~{total_params/1e9:.1f}B params")
        return arch
