...
class ReasonEvalStatCalculator(StatCalculator):
    ...
    def init(self):
        ...
        log.info(
            f"Initializing {self.reasoneval_model_path} model on device={self.device}"
        )
        self.tokenizer = AutoTokenizer.from_pretrained(
            self.reasoneval_model_path, device_map=device
        )
        if self.reasoneval_model_path.endswith("7B"):
            ...
            self.model = ReasonEval_34B.from_pretrained(
                self.reasoneval_model_path, device_map=device
            )
            self.model_size = "34B"
        else:
            raise ValueError(
                f"Could not determine model size from path: {self.reasoneval_model_path}"
            )

    def get_step_level_scores(
        self, question, reasoning_steps
    ) -> list[dict[str, float]]:
        self.init()
        PROMPT_FORMAT = "Question:\n{input}\nAnswer:\nLet's think step by step.\n"
        step_separator = f"{self.tokenizer.pad_token}"
        combined_steps = "".join(
            step.claim_text + step_separator for step in reasoning_steps
        )
        prompt = PROMPT_FORMAT.format(input=question)
        tokenized_result = self.tokenizer(prompt + step_separator + combined_steps)[
            "input_ids"
        ]
        ...
        with torch.no_grad():
            reasoning_scores = self.model(adjusted_token_ids, attention_mask)[
        ...
        return [
            {"validity": v, "redundancy": r}
            for v, r in zip(step_level_validity_scores, step_level_redundancy_scores)
        ]
...
class DirectReasonEvalScorer(UncertaintyBasedScorer):
    ...
    def compute_claim_uncertainties(
        ...
        for candidate in candidates:
            try:
                uncertainties = self._score_single_candidate(
    ...
    def _extract_question(self, trajectory: str) -> str:
        ...
        end_patterns = ["Reasoning Steps:", "Solution:", "Answer:", "- Step"]
    ...
    def _score_single_candidate(
        ...
        try:
            ...
            claims = self.steps_extractor.split_to_steps(
                candidate, candidate_tokens["input_ids"][0], self.model.tokenizer
            )
    ...
    def _compute_reasoneval_scores(
        self, question: str, claims: List[Any]
    ) -> List[float]:
        ...
        tokenized_result = self.reasoneval_tokenizer(
        ...
        with torch.no_grad():
            reasoning_scores = self.reasoneval_model(
                adjusted_token_ids, attention_mask
            )[0, labeled_token_indices, :]
            scores = torch.softmax(reasoning_scores, dim=-1).tolist()
...
class DirectReasonEvalScorerSeparate(DirectReasonEvalScorer):
    ...
    def _score_single_candidate_separate(
        ...
        # Get ReasonEval scores
        try:
            validities, redundancies = self._compute_reasoneval_scores_separate(
                question, claims
            )
            return (
                validities if validities else [0.5],
