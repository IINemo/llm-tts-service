...
class AdaptiveScalingBestOfN(StrategyBase):
    ...
    def generate_trajectory(
        ...
        for step_num in range(self.max_steps):
            ...
            log.info(
                f"Current candidate validity score: {candidate_validity_scores[0]}"
            )

            # Calculate perplexity score. In MUR, we fix perplexity as a signal to select scaled step.
            ...
            validity_scores.append(
                selected_candidate.other_data.get("validity_score", 0.0)
            )
            # Update trajectory
