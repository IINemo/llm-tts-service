#!/bin/bash
#SBATCH -J beam_search_minerva_math    # Job name
#SBATCH -N 1                           # Number of nodes
#SBATCH --ntasks-per-node=1            # 1 task
#SBATCH --gres=gpu:2                   # 2 GPUs (model + PRM scorer)
#SBATCH -p long                        # Partition/queue name
#SBATCH -t 24:00:00                    # Time limit (24h)
#SBATCH -o logs/beam_search_minerva_math_%j.out
#SBATCH -e logs/beam_search_minerva_math_%j.err
...
python scripts/run_tts_eval.py \
    --config-path=../config \
    --config-name=experiments/beam_search/minerva_math/beam_search_vllm_nothink_qwen25_7b_prm_minerva_math
