...
# class WhiteboxLogprobAdapter:
#     """Adapter that wraps a WhiteboxModel and exposes a
#     `generate_with_logprobs(request, **kwargs)` method expected by CoT-UQ.

#     This adapter does not magically compute token-level probabilities for
#     local models; instead it attempts to call available generation helpers
#     on the wrapped model and returns `token_probs=None`. It sets
#     `supports_logprobs=True` so strategies that check for the attribute
#     can proceed (they will fall back when token_probs is empty).
#     """

#     def __init__(self, whitebox_model):
#         self._wb = whitebox_model
#         # Advertise logprobs support so StrategyCoTUQ will accept this model.
#         self.supports_logprobs = True

#     def generate_with_logprobs(self, request, **kwargs):
#         """Generate a response object with `.text` and `.token_probs`.

#         The returned object mimics the BlackboxModel response used by
#         StrategyCoTUQ: it must have `text` and `token_probs` attributes.
#         token_probs will be `None` unless the wrapped WhiteboxModel exposes
#         an API producing token-level probabilities.
#         """
#         class Resp:
#             text = ""
#             token_probs = None

#         # Build a reasonably clear prompt string from the chat-like `request`.
#         # Include role labels so local models expecting plain text get clarity.
#         def build_prompt(req):
#             if isinstance(req, str):
#                 return req
#             try:
#                 # Expecting a list of dicts: [{role:..., content:...}, ...]
#                 parts = []
#                 for m in req:
...
#         # Prepare a safe set of generation parameters with sensible defaults.
#         gen_kwargs = {}
#         gen_kwargs["temperature"] = kwargs.get("temperature", 0.7)
#         gen_kwargs["max_new_tokens"] = kwargs.get("max_new_tokens", kwargs.get("max_tokens", 256))
#         gen_kwargs["top_p"] = kwargs.get("top_p", 1.0)
#         gen_kwargs["top_k"] = kwargs.get("top_k", 50)
#         # Some wrappers expect `max_tokens` instead of `max_new_tokens`
#         alt_max = {"max_tokens": gen_kwargs["max_new_tokens"]}

#         # Helper to normalize output into Resp
#         def normalize_output(out, method_name="unknown"):
#             r = Resp()
#             # dict-like
#             if isinstance(out, dict):
#                 r.text = out.get("text") or out.get("generated_text") or out.get("output") or ""
#                 r.token_probs = out.get("token_probs") or out.get("logprobs") or out.get("token_prob")
#                 log.debug(f"Whitebox adapter: normalize_output dict via {method_name}; text_len={len(r.text)}")
#                 return r
#             # plain string
#             if isinstance(out, str):
#                 r.text = out
#                 r.token_probs = None
#                 log.debug(f"Whitebox adapter: normalize_output str via {method_name}; text_len={len(r.text)}")
#                 return r
#             # object with attributes
#             if hasattr(out, "text"):
#                 r.text = getattr(out, "text") or ""
#                 r.token_probs = getattr(out, "token_probs", None)
#                 log.debug(f"Whitebox adapter: normalize_output obj via {method_name}; text_len={len(r.text)}")
#                 return r
#             # Fallback: stringify
#             r.text = str(out)
#             r.token_probs = None
#             log.debug(f"Whitebox adapter: normalize_output fallback via {method_name}; text_len={len(r.text)}")
#             return r

#         # Try several common generation interfaces and output formats. If first
#         # attempt yields empty text, retry with alternate parameter names.
#         try:
#             # 1) Preferred: generate_texts (chat/prompt style)
#             if hasattr(self._wb, "generate_texts"):
#                 # Try chat-style first
#                 try:
#                     outputs = self._wb.generate_texts(chats=[request], n=1, **gen_kwargs)
#                     if not outputs:
#                         # fallback to prompt list
#                         outputs = self._wb.generate_texts([prompt], n=1, **gen_kwargs)
#                 except TypeError:
#                     # Some wrappers don't accept 'chats' or these kw names; try prompt with alt_max
#                     try:
#                         outputs = self._wb.generate_texts([prompt], n=1, **{**gen_kwargs, **alt_max})
#                     except Exception:
#                         outputs = None

#                 if outputs:
#                     out = outputs[0]
#                     r = normalize_output(out, method_name="generate_texts")
#                     # If empty, try generation again using different kw names
#                     if not r.text.strip():
#                         log.debug("Whitebox adapter: empty text from generate_texts, retrying with alt kwarg names")
#                         try:
#                             outputs = self._wb.generate_texts([prompt], n=1, **{**gen_kwargs, **alt_max})
#                             if outputs:
#                                 r = normalize_output(outputs[0], method_name="generate_texts_retry")
#                         except Exception:
#                             pass
#                     if r.text is None:
#                         r.text = ""
...
#             # 2) generate(...) API (some wrappers expose this)
#             if hasattr(self._wb, "generate"):
#                 try:
#                     txt = self._wb.generate(request, **gen_kwargs)
#                 except TypeError:
#                     try:
#                         txt = self._wb.generate(prompt, **{**gen_kwargs, **alt_max})
#                     except Exception:
#                         txt = self._wb.generate(prompt)
#                 r = normalize_output(txt, method_name="generate")
#                 if not r.text.strip():
#                     # Retry with alternative kwarg name
#                     try:
#                         txt = self._wb.generate(prompt, **{**gen_kwargs, **alt_max})
#                         r = normalize_output(txt, method_name="generate_retry")
#                     except Exception:
#                         pass
#                 return r

#             # 3) callable model (call with prompt)
#             if callable(self._wb):
#                 try:
#                     txt = self._wb(request)
#                 except Exception:
#                     txt = self._wb(prompt)
#                 r = normalize_output(txt, method_name="callable")
#                 return r

#         except Exception as e:
#             log.debug(f"Whitebox adapter: generation attempt raised: {e}")

#         # Final fallback: empty response (explicitly logged for debugging)
#         r = Resp()
#         r.text = ""
#         r.token_probs = None
#         log.debug("Whitebox adapter: returning final empty response")
#         return r
...
#         if getattr(config, "scorer", None) is not None and getattr(config.scorer, "type", None) == "uncertainty":
#             log.info(
...
#         # Prefer BlackboxModelWithStreaming, but allow local/Whitebox models
#         # that expose `generate_with_logprobs` for compatibility with local runs.
#         # If this is a plain WhiteboxModel, wrap it with adapter that
#         # exposes `generate_with_logprobs` (token_probs will be None).
#         if isinstance(model, WhiteboxModel) and not hasattr(model, "generate_with_logprobs"):
#             model = WhiteboxLogprobAdapter(model)

#         model_ok = isinstance(model, BlackboxModelWithStreaming) or hasattr(model, "generate_with_logprobs")
#         if not model_ok:
#             raise ValueError(
#                 f"CoT-UQ requires BlackboxModelWithStreaming or a model with `generate_with_logprobs`, got {type(model).__name__}"
#             )
