    ...
    for json_file in json_files:
        try:
            with open(json_file, 'r') as f:
                ...
                if isinstance(data, dict) and ('metrics' in data or 'eval' in data or 'accuracy' in str(data)):
                    return data
...
def display_aggregated_results(aggregate_data: Dict[str, Any], all_metrics: List[Dict]):
    ...
    # Print table
    print(f"{'Metric':<35} {'Mean':>12} {'Std':>12} {'Min':>12} {'Max':>12} {'Median':>12} {'Count':>8}")
    print("-" * 100)
    ...
    for metric in display_metrics:
        ...
        print(f"{name:<35} {mean_str:>12} {std_str:>12} {min_str:>12} {max_str:>12} {median_str:>12} {count_str:>8}")
        if ci_str:
...
def save_aggregated_results(aggregate_data: Dict[str, Any], all_metrics: List[Dict], output_path: str):
...
Examples:
  ...
  python scripts/aggregate_seed_results.py outputs/2026-02-08/aime24/baseline/ -o results.json
    ...
    parser.add_argument(
        "--csv",
        action="store_true",
        help="Also save results to CSV format"
    )
